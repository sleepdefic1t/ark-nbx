...
if [ -z $1 ] || [ -z $2 ] ; then
  ...
  echo "Usage $0 <path to arkbox.conf> <step>"
...
if [ "$2" = "install" ] ; then
  ...
  echo "#--- Crontab for ArkBox-Timesave" >> $ARKBOX_FOLDER/tmp/crontab
  echo " */5 * * * * $ARKBOX_FOLDER/bin/timesave.sh $ARKBOX_FOLDER/conf/arkbox.conf save" >> $ARKBOX_FOLDER/tmp/crontab
  ...
  echo "  on OpenWrt run: /etc/init.d/arkbox enable"
